...
6. 自动对所有子目录进行 JSON 格式转换
7. 自动修改输出文件夹中 JSON 文件名以对应原图
"""
...
try:
    ...
    print(f"正在尝试导入JSON格式转换工具: {json_format_path}")
    if os.path.exists(json_format_path):
        print("找到JSON格式转换工具，开始导入...")
        with open(json_format_path, 'r', encoding='utf-8') as f:
            exec(f.read())
        print("JSON格式转换工具导入成功")
        # 检查convert_json_format是否存在
        if 'convert_json_format' in locals():
            print("convert_json_format函数已定义")
        else:
            print("警告：convert_json_format函数未定义")
...
def extract_7z_file(file_path, extract_to=None):
    ...
    try:
        ...
        # 使用subprocess调用7z命令行工具
        print(f"正在使用7z命令解压：{os.path.basename(file_path)}")
        ...
        if result.returncode == 0:
            ...
            print(f"✗ 解压失败 {file_path}: {result.stderr}")
            return None
...
def match_files_in_group(group_data, max_frames_per_group=5):
    ...
    for json_info in json_list:
        ...
        if len(jpg_no_suffix_list) == max_frames_per_group and json_parsed['suffix'] in ['speckle', 'drone'] and not is_sky_background(json_parsed['params']) and not check_bbox_size(json_info['path']):
    ...
    if fly_json_list:
        # 检查是否包含drone和speckle
        has_drone = any(item['json_info']['parsed']['suffix'] == 'drone' for item in fly_json_list)
        has_speckle = any(item['json_info']['parsed']['suffix'] == 'speckle' for item in fly_json_list)
        
        if has_drone and has_speckle:
            print(f"    ✗ fly组同时包含drone和speckle，舍弃该组")
        else:
            # 按drone和speckle分组
            fly_drone = [item for item in fly_json_list if item['json_info']['parsed']['suffix'] == 'drone']
            fly_speckle = [item for item in fly_json_list if item['json_info']['parsed']['suffix'] == 'speckle']
            
            if fly_drone:
                matched_groups.append({
                    'json_paths': [item['json_info']['path'] for item in fly_drone],
                    'json_names': [item['json_info']['name'] for item in fly_drone],
                    'annotation_jpg_paths': [item['annotation_jpg']['path'] for item in fly_drone],
                    'raw_jpg_paths': [j['path'] for j in jpg_no_suffix_list],
                    'output_subdir': 'fly/drone'
                })
                print(f"    ✓ {len(fly_drone)} 个 JSON + {len(jpg_no_suffix_list)} 张原图 -> fly/drone/")
            
            if fly_speckle:
                matched_groups.append({
                    'json_paths': [item['json_info']['path'] for item in fly_speckle],
                    'json_names': [item['json_info']['name'] for item in fly_speckle],
                    'annotation_jpg_paths': [item['annotation_jpg']['path'] for item in fly_speckle],
                    'raw_jpg_paths': [j['path'] for j in jpg_no_suffix_list],
                    'output_subdir': 'fly/speckle'
                })
                print(f"    ✓ {len(fly_speckle)} 个 JSON + {len(jpg_no_suffix_list)} 张原图 -> fly/speckle/")

    # 处理similar分类
    if similar_json_list:
        # 检查是否包含drone和speckle
        has_drone = any(item['json_info']['parsed']['suffix'] == 'drone' for item in similar_json_list)
        has_speckle = any(item['json_info']['parsed']['suffix'] == 'speckle' for item in similar_json_list)
        
        if has_drone and has_speckle:
            print(f"    ✗ similar组同时包含drone和speckle，舍弃该组")
        else:
            # 按drone和speckle分组
            similar_drone = [item for item in similar_json_list if item['json_info']['parsed']['suffix'] == 'drone']
            similar_speckle = [item for item in similar_json_list if item['json_info']['parsed']['suffix'] == 'speckle']
            
            if similar_drone:
                matched_groups.append({
                    'json_paths': [item['json_info']['path'] for item in similar_drone],
                    'json_names': [item['json_info']['name'] for item in similar_drone],
                    'annotation_jpg_paths': [item['annotation_jpg']['path'] for item in similar_drone],
                    'raw_jpg_paths': [j['path'] for j in jpg_no_suffix_list],
                    'output_subdir': 'similar/drone'
                })
                print(f"    ✓ {len(similar_drone)} 个 JSON + {len(jpg_no_suffix_list)} 张原图 -> similar/drone/")
            
            if similar_speckle:
                matched_groups.append({
                    'json_paths': [item['json_info']['path'] for item in similar_speckle],
                    'json_names': [item['json_info']['name'] for item in similar_speckle],
                    'annotation_jpg_paths': [item['annotation_jpg']['path'] for item in similar_speckle],
                    'raw_jpg_paths': [j['path'] for j in jpg_no_suffix_list],
                    'output_subdir': 'similar/speckle'
                })
                print(f"    ✓ {len(similar_speckle)} 个 JSON + {len(jpg_no_suffix_list)} 张原图 -> similar/speckle/")
    ...
    for group in matched_groups:
        ...
        for json_path in json_paths:
            ...
            try:
                ...
                if not os.path.exists(json_path):
                    print(f"    ✗ 复制 JSON 失败：{json_name} - 源文件不存在")
                    continue
        ...
        for jpg_path in annotation_jpg_paths:
            ...
            try:
                ...
                if not os.path.exists(jpg_path):
                    print(f"    ✗ 复制标注 JPG 失败：{jpg_name} - 源文件不存在")
                    continue
        ...
        for jpg_path in raw_jpg_paths:
            ...
            try:
                # 检查源文件是否存在
                if not os.path.exists(jpg_path):
                    print(f"    ✗ 复制原图 JPG 失败：{jpg_name} - 源文件不存在")
                    continue
                shutil.copy2(jpg_path, dst_jpg)
                copied_jpg += 1
            except Exception as e:
...
def rename_json_files_to_match_raw_jpg(output_dir):
    """
    修改输出文件夹中指定目录下的JSON文件名，使它们对应原图而不是标注图
    处理目录：dronefp-single、fly、similar、specle-single
    """
    target_dirs = ['dronefp-single', 'fly', 'similar', 'specle-single']
    
    print("\n" + "=" * 60)
    print("步骤 7: 修改 JSON 文件名以对应原图")
    print("=" * 60)
        ...
        for root, dirs, files in os.walk(subdir_path):
            for file in files:
                if file.lower().endswith('.json'):
                    ...
                    # 移除后缀（如 _drone 或 _speckle）
                    if '_drone' in json_name_no_ext:
                        new_name_no_ext = json_name_no_ext.rsplit('_drone', 1)[0]
                    elif '_speckle' in json_name_no_ext:
                        new_name_no_ext = json_name_no_ext.rsplit('_speckle', 1)[0]
                    else:
                        new_name_no_ext = json_name_no_ext
                    
                    new_json_name = f"{new_name_no_ext}.json"
                    ...
                    if json_path != new_json_path:
                        try:
                            ...
                            print(f"    重命名：{file} -> {new_json_name}")
                        except Exception as e:
...
def main():
    ...
    print("  8. 自动对所有子目录进行 JSON 格式转换")
    print("  9. 自动修改输出文件夹中 JSON 文件名以对应原图")
    print("=" * 60)
    ...
    # 执行JSON文件名重命名操作
    rename_json_files_to_match_raw_jpg(output_dir)
