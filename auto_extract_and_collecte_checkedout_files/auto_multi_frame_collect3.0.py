...
def match_files_in_group(group_data, max_frames_per_group=5):
    ...
    匹配规则：
    1. 检查组内是否有正好 max_frames_per_group 张无后缀原图
    ...
    # 步骤 1: 检查原图数量是否正好等于 max_frames_per_group
    if len(jpg_no_suffix_list) != max_frames_per_group:
        print(f"    ✗ 原图数量 {len(jpg_no_suffix_list)} != {max_frames_per_group}，跳过整组")
        return [], [j['name'] for j in json_list], len(jpg_list)

    print(
        f"    ✓ 原图数量 {len(jpg_no_suffix_list)} = {max_frames_per_group}，前 17 位：{jpg_no_suffix_list[0]['parsed']['ts_17']}")

    # 步骤 2: 检测混组情况（同时有 drone 和 speckle）
    ...
    if has_drone and has_speckle:
        output_subdir = 'mix_drone_speckle'
        print(f"    检测到混组 (drone + speckle) -> {output_subdir}/")
    elif has_drone:
        output_subdir = 'drone'
    elif has_speckle:
...
def main():
    print("=" * 60)
    print("多帧图片提取工具 v11 (文件名精确匹配版)")
    print("=" * 60)
    print("功能：")
    print("  1. 根据 JSON 文件名精确匹配对应的标注 JPG（同名）")
    print("  2. 根据 params+ 前 17 位匹配同一组的多帧原图（必须正好 5 张）")
    print("  3. 自动按 drone/speckle 分类输出，混组输出到 mix_drone_speckle")
    print("  4. 没有 JSON 对应的纯 JPG 组不复制")
    print("  5. 前 17 位字符不一致或原图数量不等于 5 张则整组不复制")
    print("=" * 60)
