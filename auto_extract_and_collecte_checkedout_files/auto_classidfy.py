...
def group_and_filter_files(file_groups):
    """
    对文件组进行筛选和分类
    ...
    """
    filtered_groups = {
        'single_files': [],  # 文件数量为1的组
        'multiple_files': []  # 文件数量>=2的组
    }
    ...
    for base_name, file_list in file_groups.items():
        ...
        if file_count == 1:
            filtered_groups['single_files'].append(file_list)
        else:
            filtered_groups['multiple_files'].append(file_list)
...
def analyze_json_file(json_path):
    ...
    :return: (type_value, classification_id_value) 或 (None, None) 如果解析失败
    """
    try:
        ...
        if 'DetbyStatus' in data:
            ...
            if 'efflt' in detby_status and detby_status['efflt']:
                ...
                return type_value, classification_id
...
def analyze_file_group(file_list):
    ...
    for json_file in json_files:
        ...
        type_val, class_id = analyze_json_file(json_path)
        if type_val is not None:
            type_value = type_val
            classification_id = class_id
            json_data = {'type': type_value, 'classification_id': classification_id}
            break

    # 检查是否有宽视场原图
    has_wide_original = any(
        '_wide_capture.jpg' in f['original_name'] and
    ...
    # 检查是否有斑点标注图像
    has_speckle = any('_speckle.jpg' in f['original_name'] for f in jpg_files)

    # 检查是否有无人机标注图像
    has_drone = any('_drone.jpg' in f['original_name'] for f in jpg_files)

    # 检查是否有窄视场图像
    has_narrow = any('_z[[' in f['original_name'] and '_narrow_capture.jpg' in f['original_name'] for f in jpg_files)

    # 检查是否有斑点标注JSON
    has_speckle_json = any('_speckle.json' in f['original_name'] for f in json_files)

    # 检查是否有无人机标注JSON
    has_drone_json = any('_drone.json' in f['original_name'] for f in json_files)
...
def classify_file_groups(filtered_groups):
    """
    对文件组进行分类
    :param filtered_groups: 包含筛选结果的字典
    :return: 分类后的字典，键为分类名称，值为文件组列表
    """
    classified_groups = {
        '未检出_背景': [],  # 文件数量为1的组，且没有JSON或JSON解析无type
        '已检出_斑点': [],  # 有JSON且type为speckle
        '已检出_分类模型_红框正确上报': [],  # 有JSON且type为drone且classification_ID为drone
        '已检出_分类模型_黑框正确未上报': [],  # 有JSON且type为drone但classification_ID不为drone
    }

    # 1. 处理文件数量为1的组
    for file_list in filtered_groups['single_files']:
        analysis = analyze_file_group(file_list)

        if analysis['has_json'] and analysis['type']:
            # 如果有JSON且能解析出type，根据type分类
            ...
            # 没有JSON或解析失败，归为未检出_背景
            classified_groups['未检出_背景'].append(file_list)

    # 2. 处理文件数量>=2的组
    for file_list in filtered_groups['multiple_files']:
        analysis = analyze_file_group(file_list)
...
def copy_files_to_categories(classified_groups, dir_paths):
    ...
    for category_name, groups in classified_groups.items():
        ...
        for file_list in groups:
            ...
            # 分析文件组
            analysis = analyze_file_group(file_list)
            type_value = analysis['type']
            classification_id = analysis['classification_id']

            print(f"  组 {group_idx}: 基本名称='{base_name}', 文件数={file_count}, "
                  f"类型={type_value}, 分类ID={classification_id}, "
                  f"宽视场原图={analysis['has_wide_original']}, 窄视场={analysis['has_narrow']}")
            ...
            group_info.append({
                ...
                'has_narrow': analysis['has_narrow'],
                'has_speckle': analysis['has_speckle'],
                'has_drone': analysis['has_drone']
            })
...
def create_operation_log(output_dir, group_info, classified_groups, total_files, target_dir, dir_paths):
    ...
    with open(log_path, 'w', encoding='utf-8') as f:
        ...
        for info in group_info:
            ...
            f.write(f"  窄视场图像: {'有' if info['has_narrow'] else '无'}\n")
...
def main():
    ...
    print("=" * 60)
    print("分类规则:")
    print("  1. 文件组只有1个文件且无有效JSON -> 未检出_背景")
    print("  2. 文件组有JSON且type为speckle -> 已检出_斑点")
    print("  3. 文件组有JSON且type为drone且classification_ID为drone -> 已检出_分类模型_红框正确上报")
    print("  4. 文件组有JSON且type为drone但classification_ID不为drone -> 已检出_分类模型_黑框正确未上报")
    print("=" * 60)
    ...
    print("步骤3.5: 筛选文件组")
    print("=" * 60)
    filtered_groups = group_and_filter_files(file_groups)

    # 统计每组文件数量
    print(f"  1个文件的组: {len(filtered_groups['single_files'])}")
    print(f"  2个及以上文件的组: {len(filtered_groups['multiple_files'])}")
    ...
    print("步骤5: 分类文件组")
    print("=" * 60)
    classified_groups = classify_file_groups(filtered_groups)
