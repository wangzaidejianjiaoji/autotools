...
"""
Web SSH 工具 - 基于 Flask 和 WebSocket
让同事可以通过浏览器访问操作 SSH 命令
cd autotools\auto_UX_linux
python web_ssh_tool.py
"""
...
def update_command(name):
    ...
    for cmd in custom_commands:
        if cmd['name'] == name:
            cmd['name'] = data.get('name', cmd['name'])
            cmd['command'] = data.get('command', cmd['command'])
            cmd['need_password'] = data.get('need_password', cmd.get('need_password', False))
            save_custom_commands(custom_commands)
...
<body>
            ...
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        <button class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#editCommandModal">
                            ✏️ 编辑命令
                        </button>
                    </div>
                </div>
    ...
    <div class="modal fade" id="editCommandModal" tabindex="-1" aria-labelledby="editCommandModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editCommandModalLabel">编辑自定义命令</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="commandEditList">
                        <!-- 命令编辑项将通过 JavaScript 动态生成 -->
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function loadCommands() {
            fetch('/api/commands')
                ...
                .then(data => {
                    ...
                    // 加载命令编辑列表
                    const commandEditList = document.getElementById('commandEditList');
                    commandEditList.innerHTML = '';

                    data.forEach(cmd => {
                        const commandItem = document.createElement('div');
                        commandItem.className = 'mb-3 p-3 border rounded';
                        commandItem.innerHTML = `
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <h6>\${cmd.name}</h6>
                                <button class="btn btn-danger btn-sm delete-command-btn" data-name="\${cmd.name}">
                                    删除
                                </button>
                            </div>
                            <p><strong>命令:</strong> \${cmd.command}</p>
                            <p><strong>需要密码:</strong> \${cmd.need_password ? '是' : '否'}</p>
                        `;
                        commandEditList.appendChild(commandItem);
                    });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('saveServerBtn 元素:', saveServerBtn);
            
            const saveCommandBtn = document.getElementById('saveCommandBtn');
            console.log('saveCommandBtn 元素:', saveCommandBtn);
            ...
            // 终止按钮
            document.getElementById('terminateBtn').addEventListener('click', terminateCommand);
            ...
            if (saveServerBtn) {
                saveServerBtn.addEventListener('click', function() {
                    ...
                    console.log('服务器信息:', {name, host, port, username, password});
            ...
            if (saveCommandBtn) {
                saveCommandBtn.addEventListener('click', function() {
                    ...
                    console.log('命令信息:', {name, content, needPassword});
...
if __name__ == '__main__':
    ...
    print("\n启动命令:")
    print("cd autotools\auto_UX_linux")
    print("python web_ssh_tool.py")
    print("\n访问地址:")
    print("http://localhost:5000")
    print("\n如果要让同事访问，请使用您的 IP 地址:")
    print("例如: http://192.168.87.106:5000/")
